...
| 날짜 | 변경 내용 |
|---|---|
| 2026-05-05 (v14) | **#13 신규** — 확정신고 누진 적용 v3 본격 도입 (법 제104조 ⑤ 정확본). 사용자 38·39번째 짚음 정정 (38: 확정신고 누진 적용 미반영 / 39: 시행령 제167조의10 잘못 인용). 명세서 v0.3-B §5-7 신규 + §9-2·§9-4 보강 영속화 (5/5 17:00). 본 관제탑 자체 위반 누적 29~43 (5/3~5/5 누적 15건). 의사결정 #13에 산식 본문 + 회귀 안전성 + 반영 영역 7단계 + 본 관제탑 학습 영속화. |
...
### #13. 확정신고 누진 적용 v3 본격 도입 (법 제104조 ⑤ 정확본)

- **결정 시점**: 2026-05-05 17:00
- **결정 본질**: 동일 과세기간(과세연도) 다중 양도 시 양도소득 산출세액 산식을 법 제104조 ⑤ 정확본으로 본격 도입
- **사용자 짚음 본질** (38·39번째):
  - 38번째 (5/4 21:00): 본 관제탑 v0.3-A 산식이 양도 1건별 단독 누진세율(예정신고 산식)만 적용. 동일 과세기간 다중 양도 시 확정신고 누진 적용 본격 처리 미반영
  - 39번째 (5/4 22:00): 본 관제탑 v2 정정안에서 시행령 제167조의10을 "다주택 중과 자산은 자산별 단독 과세 근거"로 잘못 인용. 시행령 본문은 중과 대상 주택 정의 조문일 뿐. 법 제104조 ⑤ 1호·2호 모두 제7항(중과)을 배제하는 근거 없음
- **법령 정확본 산식**:
  - 1호 (합산 누진): 모든 자산 과세표준 합산 → 제55조제1항 일반 누진세율 (가산세율 미적용)
  - 2호 (자산별 단독 합계 + 단서):
    - 단서: 동일 호 세율 적용 자산 ≥ 2 → 그 자산들 과세표준 합산 후 호별 세율 + MAX(합산, 단독) 그룹별 채택
    - 단서 미발동 자산: 단독 그대로
  - 산출세액 = MAX(1호, 2호)
- **v0.3-A 회귀 안전성**:
  - 단일 양도 (TC-001~014) → `applyFinalReturnV3` 진입 시 `length === 1` → `SINGLE_TRANSFER` 분기 → calculatedTax 그대로 → 회귀 영향 없음
  - KPI 5자 일치 100% 누적 14건 보존 보장
- **반영 영역** (단계별 진행):
  1. 명세서 v0.3-B §5-7 신규 (5/5 17:00 영속화 완료)
  2. 명세서 v0.3-B §9-2·§9-4 보강 (issueFlag 2종 신규)
  3. 의사결정 #13 신규 (본 결정)
  4. 모듈 스펙 v0.3-B 신규 작성 (별도 작업 창 — 의사결정 #9 v9)
  5. 작업지시서 v0.3-B 신규 작성
  6. Claude Code 코드 산출 (tax_rules.js + tax_engine.js v0.3-B 보강)
  7. 회귀 검증 + GitHub Pages 라이브 검증
- **검증 영역** (검증팀 본격 검증 권고):
  - TC-S05·S06·S07 v3 결과값 손계산 + 홈택스 모의계산 일치 여부
  - 1호 채택 발동 케이스 (모든 자산 일반과세 + 양도차익 작은) 본격 검증
  - 단서 적용 사례 (동일 호 세율 자산 ≥ 2) 검증
- **본 관제탑 자체 위반 누적 학습** (5/3~5/5 누적 14건):
  - 29: §10 하위 구조 정독 누락
  - 30: 검증 양식의 본질 가치 (비교 가능 영역)
  - 31: 1세대1주택 비과세 양도차익 분기
  - 32: 양도차익 모든 분기 본질
  - 33: 추천 시나리오 결정 영역 vs 결과 설명 레이어
  - 34: MVP 단일 지표 vs post-MVP 본질 가치
  - 35: effectiveTaxRate 정렬의 영역 한계
  - 36: §섹션 표 본문 + 매핑 영역 통합 정독
  - 37: 사용자 질문 파일명·영역 정독
  - 38: 동일 과세기간 다중 양도 + 확정신고 누진 적용
  - 39: 시행령 조문 인용 시 본문 정독 우선
  - 40: 룰·모듈·코드·명세서·로그 통합 영역 사전 명시
  - 41: 모듈 스펙(단일 진본) vs 작업지시서(분리본) 영역 구분
  - 42: 정정 단계 vs 신규 작성 단계 도약 회피
  - 43: 본 관제탑 메모리 추측 vs 실제 .md 영속화 검증 (본 결정 시 정정)
- **영향 범위**: 명세서 v0.3-B, 모듈 스펙 v0.3-B (신규), 작업지시서 v0.3-B (신규), tax_rules.js + tax_engine.js v0.3-B, 회귀 테스트, GitHub Pages 라이브 검증, 5/7 발표
- **연관**: 의사결정 #5 (코드 파일 구조), #6 (산출물 영속화), #9 v9 (작업 창 분담), #10 D안 (시나리오 비교 정렬), #11 (정확성 > 속도), #12 (TaxOpt 본질 가치)
- **본질 가치 영역**: MVP v0.3-B는 양도 시점 단일 양도소득세 비교 본격 처리. 양도하지 않은 자산의 미래 보유 가치(1주택 비과세·가격·보유세·NPV)는 post-MVP B-028~B-031 본질 가치 4영역

### #14. getRateGroupKey 반환 형식 정수 채택 + 모듈 스펙 본문 정정 인계

- **결정 시점**: 2026-05-06
- **결정 본질**: 5/4 합의 결정 1번 "정수 키 채택" 본문 정합 영역에서 모듈 스펙 v0.3-B §5-7-3 line 599~602 영속화 본 (문자열 반환)과 충돌 영역 발견 → 정수 반환 단일 채택 + 모듈 스펙 정정 인계
- **사용자 짚음 본질** (본 관제탑 검증 갭 점검 6건 결과 — 짚을 부분 1번): 작업지시서 v0.3-B 합본 v1.0 산출 후 본 관제탑 검증 시 발견. 영속화 차단 영역
- **결정 본문**:
  - `getRateGroupKey` 반환 형식 — **정수 반환** (`return Math.round(addition * 100)`) 채택
  - 사유: (1) 5/4 합의 결정 1번 본문 정합 (2) R-B-6 회귀 그룹 정수 비교 정합 (`assert.strictEqual(taxEngine.getRateGroupKey(0.20), 20)`) (3) Map 키 비교 영역 부동소수점 회피 (B-022 영역)
  - 모듈 스펙 정정 인계 영역: `tax_engine.md` v0.3-B §5-7-3 line 599~602 (문자열 반환) → 정수 반환으로 정정 권고
- **본 작업지시서 영역**: §4-3-3 line 509 본문에 정정 인계 영역 명시 + Claude Code 단일 진본 채택 명시 (모듈 스펙과 충돌 시 본 작업지시서 우선)
- **권고 처리 시점**:
  - (a) 작업지시서 영속화 후 작업 창 #15 후속 보강으로 모듈 스펙 정정 (권고)
  - (b) post-MVP 보강
- **연관**: 의사결정 #5 강화 (단일 소스 원칙), #6 (영속화 의무), #11 (정확성 > 속도), #13 (확정신고 v3 산식 — 본 작업지시서 본문 영역)
- **영향 범위**: 모듈 스펙 `tax_engine.md` v0.3-B §5-7-3, 작업지시서 v0.3-B (본 작업지시서), Claude Code 진입 단계
- **본 관제탑 자체 위반 누적 학습** (5/3~5/6 누적 15건):
  - ... (1~14 그대로)
  - 44: 작업 창 #15 모듈 스펙 산출 시 `getRateGroupKey` 반환 형식 영속화 영역 자체 검증 누락 — 작업 창 #16 진입 후 본 관제탑 검증에서 발견

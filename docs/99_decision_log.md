...
├── js/
│   └── tax_rules.js                      ← v0.1.1 (4/28 영속화)
├── tests/
│   └── tax_rules.test.js                 ← 회귀 테스트 (4/28 영속화)
├── docs/
│   ├── v0.1/
...
- **세부 사항**:
  ...
  - v0.1.0 → v0.1.1로 10건 패치(P1~P10) 반영: 보유기간 365.25 근사 제거, 절사 정책 명시, 입력 검증 7개 명시, 세후 매각금액 정의, 비교산출세액 미적용, issueFlag 2개 추가, 골든셋 분리, 비조정지역 가정 명시, OUT_OF_V01_SCOPE_REGULATED_AREA issueFlag, 12→13단계 (적용 세율 단계 신설)
- **명명 통일** (v5 보강): "Codex" → "Code" 일괄 정정
- **영향 범위**: 명세서·코드·검증·발표 데모 전체

### #9. 작업지시서의 사후 운명 (신규 — 2026-04-29)

- **결정일**: 2026-04-29
- **결정**: 작업지시서가 작성된 후 다른 작업 창에서 동일 산출물을 선행 완성한 경우, 해당 작업지시서는 **검증·확인 작업으로 재해석**한다.
- **배경**:
  - 작업지시서 01(`tax_rules.js`)이 4/28 작업 창 #1-1에서 명세서 검증 후 직접 완성됨
  - 원래는 Claude Code가 받아서 만들 작업이었으나 선행됨
  - 4/29 작업 창 #1-1 진입 시 작업지시서 01의 처리 방향 결정 필요
- **세부 적용**:
  - 작업지시서 01을 폐기하지 않음 (산출물의 가치 보존)
  - Claude Code가 받아서 다음을 수행: (1) `tax_rules.js`와 `tax_rules.test.js`를 받아 회귀 테스트 자동 실행, (2) 명세서 §1-3·§4-2·§4-3과의 정합성 점검, (3) 모듈 스펙과의 일치 확인, (4) 결과 보고
  - Claude Code의 첫 본격 도입을 "검증" 차원으로 시작 → 안전한 학습 곡선
- **일반 원칙**:
  - 향후 작업지시서 02·03에도 동일 상황 발생 시 적용
  - 작업지시서 자체가 잘못 작성된 경우는 폐기 후 재작성 (별도 처리)
- **영향 범위**: 4/29 이후 작업지시서 처리 흐름, Code 도입 시점
...
| 산출물 | 상태 | 영속화 |
|---|---|---|
| 의사결정 로그 v7 | ✅ 본 파일 | docs/99_decision_log.md (Claude.ai 업로드 필요) |
...
| **tax_rules.js v0.1.1** | ✅ **selfTest() ok: true 검증** | js/tax_rules.js |
| **tax_rules.test.js** | ✅ **회귀 테스트 작성** | tests/tax_rules.test.js |
| 작업지시서 01 (tax_rules) | ✅ 작성 완료 (재해석 — 검증 작업) | docs/05_code_work_orders/01_tax_rules.md |
...
| v0.2 명세서 (장특공·비과세) | ❌ 미착수 (4/29 저녁 시작 예정) | docs/v0.2/ |
...
| 4/29 (수) ⏳ | 의사결정 로그 v7, 작업지시서 01 Code 검증 실행, 작업지시서 02 작성·실행 | **v0.1 코드 완료 목표** |

# TaxOpt 테스트 케이스 v0.2.0 (초안 — TC-006~010 신규)
...
| 작성일 | 2026-04-30 |
| 관련 문서 | `docs/v0.2/01_calc_engine_spec.md`, `docs/v0.2/03_input_schema.md`, `docs/v0.2/modules/tax_engine.md` |
...
검증팀 손계산 후 명세서 v0.2.0 → v0.2.1 검증 통과 확정 예정.
...
|   | `holdingRate` (표 2) | 0.32 | 8 × 0.04 |
|   | `residenceRate` (표 2) | 0.32 | 8 × 0.04 |
...
| 10 | `calculatedTax` | (검증팀 손계산) | 840,000 + (45,740,000 − 14,000,000) × 0.15 = 840,000 + 4,761,000 = **5,601,000** |
| 11 | `localIncomeTax` | (검증팀 손계산) | floor(5,601,000 × 0.1) = **560,100** |
| 12 | `totalTax` | (검증팀 손계산) | **6,161,100** |
| 13 | `netAfterTaxSaleAmount` | (검증팀 손계산) | 1,500,000,000 − 6,161,100 = **1,493,838,900** |

> 위 10~13단계 값은 Claude의 산식 산출 추정치. **검증팀 손계산으로 정답 확정 필요**.
...
|   | `holdingRate` (표 1) | 0.24 | 0.06 + (12-3) × 0.02 |
...
| 10 | `calculatedTax` | (검증팀) | 94,060,000 + (362,300,000 − 300,000,000) × 0.40 = 94,060,000 + 24,920,000 = **118,980,000** |
| 11 | `localIncomeTax` | (검증팀) | floor(118,980,000 × 0.1) = **11,898,000** |
| 12 | `totalTax` | (검증팀) | **130,878,000** |
| 13 | `netAfterTaxSaleAmount` | (검증팀) | 1,000,000,000 − 130,878,000 = **869,122,000** |
...
| 4 | `holdingYears` | 10 | |
|   | `residenceYears` | 10 | |
|   | `holdingRate` (표 2) | 0.40 | (10년 이상 클램프) |
|   | `residenceRate` (표 2) | 0.40 | (10년 이상 클램프) |
|   | `totalRate` | 0.80 | (최대 80%) |
|   | `appliedDeductionTable` | 2 | |
...
| 10 | `calculatedTax` | (검증팀) | 840,000 + (16,785,715 − 14,000,000) × 0.15 = 840,000 + 417,857 = **1,257,857** |
| 11 | `localIncomeTax` | (검증팀) | floor(1,257,857 × 0.1) = **125,785** |
| 12 | `totalTax` | (검증팀) | **1,383,642** |
| 13 | `netAfterTaxSaleAmount` | (검증팀) | 1,400,000,000 − 1,383,642 = **1,398,616,358** |

> 검증팀 손계산 + 홈택스 모의계산 교차 확인 필요.
...
|   | `appliedDeductionTable` | 1 | |
|   | `holdingRate` (표 1) | 0.10 | 0.06 + (5-3) × 0.02 |
...
| 10 | `calculatedTax` | (검증팀) | 94,060,000 + (344,000,000 − 300,000,000) × 0.40 = 94,060,000 + 17,600,000 = **111,660,000** |
| 11 | `localIncomeTax` | (검증팀) | floor(111,660,000 × 0.1) = **11,166,000** |
| 12 | `totalTax` | (검증팀) | **122,826,000** |
| 13 | `netAfterTaxSaleAmount` | (검증팀) | 1,000,000,000 − 122,826,000 = **877,174,000** |
...
## 8. 검증 결과 종합 (검증팀 회신 후 채움)

| TC | 검증팀 손계산 | Claude 명세서 산출 | 홈택스 모의계산 | 결과 |
|---|---|---|---|---|
| TC-006 | (대기) | 0 | (대기) | (대기) |
| TC-007 | (대기) | 6,161,100 | (대기) | (대기) |
| TC-008 | (대기) | 130,878,000 | (대기) | (대기) |
| TC-009 | (대기) | 1,383,642 | (대기) | (대기) |
| TC-010 | (대기) | 122,826,000 | (대기) | (대기) |

# tax_engine.js 모듈 스펙 v0.2.0

| 항목 | 내용 |
|---|---|
| 문서 ID | `docs/v0.2/modules/tax_engine.md` |
| 버전 | v0.2.0 |
| 상태 | 작성 완료 (2026-04-30, 검증 전) |
...
| 관련 작업지시서 | `docs/05_code_work_orders/04_tax_engine_v0.2.md` (예정) |
| 관련 명세서 | `docs/v0.2/01_calc_engine_spec.md` v0.2.0 |
| 관련 입력 스키마 | `docs/v0.2/03_input_schema.md` v0.2.0 |
| 관련 골든셋 | `docs/v0.2/06_test_cases.md` (TC-006~010) + `docs/v0.1/06_test_cases.md` (TC-001~005, 회귀) |
| 의존 모듈 스펙 | `docs/v0.2/modules/tax_rules.md` v0.2.0 (예정, 장특공 표 상수 추가) |
| 관련 의사결정 | `docs/99_decision_log.md` #5, #8, #9 v9, #10 |
| 관련 백로그 | B-008 (effectiveTaxRate 사전 노출, v0.1에서 처리), B-009 (1세대1주택 비과세 — v0.2에서 처리) |
...
명세서 §5-2·§5-3 산식을 그대로 옮긴다.

| 항목 | 내용 |
|---|---|
| 입력 | `{ taxableGain, holdingYears, residenceYears, is1Se1House, isHighValueHouse }` |
| 출력 | `{ longTermDeduction: number, appliedDeductionTable: 1\|2\|null, holdingRate: number, residenceRate: number, totalRate: number }` |
| 산식 | (a) **표 적용 자격 판정**: `is1Se1House && isHighValueHouse && holdingYears >= 3`이면 표 2. 그 외 다주택 또는 1세대1주택 보유<3년은 표 1. 단계 2에서 종료된 12억 이하 비과세는 본 함수에 도달하지 않음(상위 분기). (b) **표 1**: `holdingYears < 3` → `holdingRate=0`, `residenceRate=0`, `totalRate=0`, `appliedDeductionTable=null`. `3 ≤ holdingYears < 15` → `holdingRate = 0.06 + (holdingYears − 3) × 0.02`, `residenceRate=0`, `totalRate=holdingRate`, `appliedDeductionTable=1`. `holdingYears >= 15` → `holdingRate=0.30`, `residenceRate=0`, `totalRate=0.30`, `appliedDeductionTable=1`. (c) **표 2**: `holdingRate = (holdingYears>=10) ? 0.40 : holdingYears × 0.04`. `residenceRate = (residenceYears<2) ? 0 : (residenceYears<3) ? 0.08 : (residenceYears>=10) ? 0.40 : residenceYears × 0.04`. `totalRate = holdingRate + residenceRate` (최대 0.80, 산식상 자동 보장). `appliedDeductionTable=2`. (d) `longTermDeduction = Math.floor(taxableGain × totalRate)`. |
| 절사 | `longTermDeduction`만 floor. 공제율은 비율(절사 없음). |
| 부수효과 | 없음 |
| 결정성 | 동일 입력 → 동일 출력 |
| 예외 | `taxableGain < 0` 시 throw (단계 1에서 손실 검출되어야 함) |

> **표 2 거주공제율 경계 (2~3년 미만 8%)**: 시행령 제95조 ② 표 2 우측의 "보유기간 3년 이상으로서 거주 2~3년 미만 8%" 조건은 본 산식에서 표 2 적용 자격이 이미 `holdingYears >= 3`을 보장하므로, 본 함수는 `residenceYears` 단독 분기로 8% 적용 가능 (보유 조건 중복 검사 불필요).

> **공제율 합계 80% 상한**: 표 2 산식은 `holdingRate(≤0.40) + residenceRate(≤0.40) = ≤0.80`이므로 산식상 자동 상한. 별도 `Math.min(0.80, ...)` 가드 불필요하나, 방어적으로 추가해도 무방.
...
### 8-1. tax_rules.js 사용 항목 (v0.2 보강)
...
**v0.2에서 추가 의존하는 멤버** (작업지시서 03 또는 04에서 `tax_rules.js`에 산출 예정):

| 사용 멤버 | 사용 단계 | 형태 (권고) |
|---|---|---|
| `HIGH_VALUE_HOUSE_THRESHOLD` | 단계 2·3 (12억 비교) | number, `1_200_000_000` |
| `LONG_TERM_DEDUCTION_TABLE_1` | 단계 4 (표 1 산출) | object[] (보유연수→공제율 8행) **또는** function `getTable1Rate(holdingYears)` |
| `LONG_TERM_DEDUCTION_TABLE_2_HOLDING` | 단계 4 (표 2 보유공제율) | object[] (보유연수→공제율 8행) **또는** function `getTable2HoldingRate(holdingYears)` |
| `LONG_TERM_DEDUCTION_TABLE_2_RESIDENCE` | 단계 4 (표 2 거주공제율) | object[] (거주연수→공제율 9행) **또는** function `getTable2ResidenceRate(residenceYears)` |
| `EXEMPTION_HOLDING_THRESHOLD_YEARS` | 단계 2 (비과세 보유요건) | number, `2` |
| `EXEMPTION_RESIDENCE_THRESHOLD_MONTHS` | 단계 2 (조정대상지역 거주요건) | number, `24` |

> 본 모듈 스펙은 `tax_rules.js`의 표 상수 형태(배열 vs 함수)를 강제하지 않는다. `tax_rules.md` v0.2 갱신본이 결정한다.

> **단계 4 산출의 단일 정본**: 표 1·2 산식은 명세서 §5-2·§5-3에 산식 형태(연차 × 비율)로 이미 확정되어 있다. `tax_rules.js`가 표 형태를 노출하든 산식 함수를 노출하든, **산출값은 명세서 정본과 일치**해야 한다. 검증팀 손계산이 표 형태(법령 원본)를 채택했으므로, 산식 산출과 표 룩업이 모든 정수 연차에서 일치하는지 회귀 테스트로 검증한다.
...
1. **`tax_rules.js` v0.2의 노출 형태 (배열 vs 함수)**: 표 룩업 vs 산식 산출 중 어느 쪽을 정본으로 할지. 작업지시서 03(또는 04) 작성 시 결정. 본 문서는 양쪽 모두 허용하도록 표기.
2. **`selfTest()` sanity 케이스 추가 (TC-006·008·010)**: 부트스트랩 부담 검토 후 채택 여부 결정. 일단 권장으로 표기.
3. **`Object.freeze` 적용 여부**: v0.3 시나리오 엔진 도입 시 모듈 격리 요구 여부에 따라 결정.
4. **`HOLDING_PERIOD_BOUNDARY` 확장 임계치 (2년·3년·15년 ±3일)**: 명세서 §6-3 확장은 확정. 본 모듈 스펙은 `collectIssueFlags`의 입력 `intermediates`에 `holdingYears` 추가만 명시. 정수 임계 외에 일자 단위 임계도 필요한지(±3일은 일자 기준)는 검증팀 의견 수렴 후 결정.
5. **`UNREGISTERED_ASSET_ASSUMED_FALSE` → `UNREGISTERED_RATE_NOT_APPLIED` 이름 변경**: 명세서 §6-2 권고. 본 모듈 스펙은 새 이름을 채택하나, 결정 미확정 시 v0.1 이름 그대로 유지 가능. 작업지시서 단계에서 최종 결정.
